...
const trendingPaging = (movies, page, limit) => {
  ...
  const results = {};
  const totalMovies = movies.sort((a, b) => b.popularity - a.popularity);
  results.page = page;
  results.results = totalMovies.slice(startIndex, endIndex);
  results.total_page = totalMovies.length / limit;
  return results;
};

const topRatingPaging = (movies, page, limit) => {
  const startIndex = (page - 1) * limit;
  const endIndex = page * limit;
  const results = {};
  const totalMovies = movies.sort((a, b) => b.vote_average - a.vote_average);
  results.page = page;
  results.results = totalMovies.slice(startIndex, endIndex);
  results.total_page = totalMovies.length / limit;
  return results;
...
const genresPaging = (movies, page, limit, genreId) => {
  ...
  const totalMovies = movies.filter((movie) => {
    if (movie.genre_ids && movie.genre_ids.includes(genreId)) {
      return movie;
    }
  });
  ...
  results.page = page;
  results.results = totalMovies.slice(startIndex, endIndex);
  results.total_page = Math.ceil(totalMovies.length / limit);
...
const searchPaging = (movies, page, limit, query) => {
  ...
  const totalMovies = movies.filter((movie) => {
    ...
    if (hasOverview || hasTitle) return movie;
  });
  results.page = page;
  results.results = totalMovies.slice(startIndex, endIndex);
  results.total_page = Math.ceil(totalMovies.length / limit);
